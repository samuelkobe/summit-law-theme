...
// Get all areas for the filter (only those with insights/posts)
...
// Calculate counts for each area (only posts, not cases)
$area_counts = array();
if ( $all_areas && ! is_wp_error( $all_areas ) ) {
	...
	// Filter out areas with no posts
...
// Get archive title
$archive_title = $posts_page ? get_the_title( $posts_page_id ) : __( 'Insights', 'summit-law-theme' );
...
<header class="page-header bg-green-deep lg:h-[30dvh] min-h-[240px] lg:min-h-[320px] flex items-center pt-12 pb-20 lg:py-24">
	<section class="banner-after-content h-full flex flex-col justify-center container mx-auto p-6 relative text-left">
		...
		<?php if ( $posts_page && $posts_page->post_excerpt && ! $current_area_name ) : ?>
	...
	<div class="container mx-auto flex flex-row justify-between">
		...
		<nav aria-label="Back navigation" class="md:hidden py-4">
			<?php if ( $current_area_name ) : ?>
				<!-- When filtering by area, back link goes to Insights -->
		...
		<nav aria-label="Breadcrumb" class="hidden md:block py-8">
			<ol class="flex items-center gap-2 text-sm font-semibold">
				...
				<?php if ( $current_area_name ) : ?>
					<!-- Insights link when filtering by area -->
					<li>
				...
				<?php else : ?>
					<!-- Insights as current page when not filtering -->
					<li aria-current="page" class="text-green-deep">
						<?php echo esc_html( $archive_title ); ?>
					</li>
				<?php endif; ?>
		...
		<div class="py-4 md:py-8 flex flex-row gap-x-4">
			...
			<ul class="social-share flex flex-row items-center gap-2">
				...
				$page_url   = urlencode( home_url( '/insights/' ) );
				$page_title = $posts_page ? urlencode( get_the_title( $posts_page_id ) ) : urlencode( __( 'Insights', 'summit-law-theme' ) );
				...
				<aside class="lg:col-span-4 xl:col-span-3">
					...
					<button
						...
						<span class="font-semibold text-brand-black">
							<?php esc_html_e( 'Filter Insights', 'summit-law-theme' ); ?>
						</span>
				...
				<div class="lg:col-span-8 xl:col-span-9">
					...
					<div class="flex flex-wrap items-center justify-between gap-4 mb-8">
						<h2 class="h3 border-b-2 lg:border-b-[3px] border-brand-30 w-fit pb-[6px] lg:pb-[10px]">
							...
							printf(
								esc_html( _n( '%s Insight', '%s Insights', $found, 'summit-law-theme' ) ),
					...
					<?php else : ?>
						...
						<div class="text-center py-12 bg-brand-10 rounded-lg">
							...
							<h3 class="h4 text-brand-black mb-2">
								<?php esc_html_e( 'No insights found', 'summit-law-theme' ); ?>
							</h3>
							<p class="text-brand-50 mb-6 max-w-md mx-auto">
								<?php esc_html_e( 'No insights found matching your criteria. Try adjusting your filters.', 'summit-law-theme' ); ?>
							</p>
							<?php if ( $current_area ) : ?>
