<?php
/**
 * Role Taxonomy Archive Template
 *
 * Displays team members filtered by role (attorney, clerk, administration, etc.)
 * URL: /role/{role-slug}/
 *
 * Uses same styling as archive-team.php with bg-brand-black for team member photos
 */

get_header();

// Get current role term
$current_term = get_queried_object();
$role_name    = $current_term->name;
$role_slug    = $current_term->slug;

// Role order for sorting within the archive
...
<main id="main" class="site-main team-archive">
			...
			<?php else : ?>
				<p class="text-white text-lg lg:text-xl w-full md:w-3/4 2xl:w-2/3">
					...
					printf(
						...
						esc_html__( 'Meet our %s team members.', 'summit-law-theme' ),
						esc_html( strtolower( $role_name ) )
					);
					?>
				</p>
			<?php endif; ?>
	...
	<section class="bg-white">
		<div class="container mx-auto flex flex-col md:flex-row md:justify-between">
			...
			<nav aria-label="Back navigation" class="md:hidden py-4">
				<a href="<?php echo esc_url( home_url( '/team/' ) ); ?>" class="inline-flex items-center gap-2 text-sm font-semibold text-brand-40 hover:text-green-deep transition-colors">
					...
					<?php esc_html_e( 'Team', 'summit-law-theme' ); ?>
				</a>
			</nav>
			...
			<nav aria-label="Breadcrumb" class="hidden md:block py-8">
				<ol class="flex items-center gap-2 text-sm font-semibold">
					...
					<li>
						<a href="<?php echo esc_url( home_url( '/team/' ) ); ?>" class="text-brand-40 hover:text-green-deep transition-colors">
							<?php esc_html_e( 'Team', 'summit-law-theme' ); ?>
						</a>
					</li>
					<li aria-hidden="true">
			...
			<div class="py-4 md:py-8 flex flex-row gap-x-4">
				...
				<ul class="team-social flex flex-row items-center gap-2">
					...
					$page_url   = urlencode( get_term_link( $current_term ) );
					$page_title = urlencode( $role_name . ' - ' . get_bloginfo( 'name' ) );

					// Define share links
					$share_links = array(
						'linkedin' => 'https://www.linkedin.com/sharing/share-offsite/?url=' . $page_url,
						'x'        => 'https://twitter.com/intent/tweet?url=' . $page_url . '&text=' . $page_title,
						'facebook' => 'https://www.facebook.com/sharer/sharer.php?u=' . $page_url,
						'email'    => 'mailto:?subject=' . get_bloginfo( 'name' ) . ' - ' . $page_title . '&body=Check out our ' . strtolower( $role_name ) . ' team: ' . $page_url,
					);
	...
	<section class="bg-white w-full min-h-[800px] py-12 lg:pb-24 lg:pt-16 max-md:pt-2">
		<div class="team-grid container mx-auto px-4 pb-16">

			<?php
			// Custom query to order by ACF Role field hierarchy, then alphabetically
			$role_order = array( 'partner', 'associate', 'clerk', 'assistant', 'admin' );

			// Get all team members in this role taxonomy term
			$team_args = array(
				'post_type'      => 'team',
				'posts_per_page' => -1,
				'orderby'        => 'title',
			...
			// Group posts by ACF role field value
			$posts_by_role = array();
			...
			foreach ( $posts_by_role as $role => $posts ) {
				if ( ! in_array( $role, $role_order, true ) ) {
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-12 gap-x-8 gap-y-12 lg:gap-y-16 2xl:gap-y-24">
					...
					<article class="team-member-card <?php echo esc_attr( $xl_col_class ); ?>">
						...
						<a href="<?php the_permalink(); ?>" class="group block">
							...
							<div>
								<div>
									<h2 class="h2-line-after h2 font-museum capitalize text-green-deep underline decoration-2 decoration-transparent group-hover:decoration-green-deep underline-offset-4 transition-colors duration-300"><?php the_title(); ?></h2>
								</div>
						...
						<div class="mt-2 lg:mt-4">
							<a href="<?php the_permalink(); ?>" class="text-brand-40 hover:text-brand-black text-sm lg:text-base font-bold underline decoration-2 decoration-transparent hover:decoration-brand-black transition-colors duration-300"><?php esc_html_e( 'View full profile', 'summit-law-theme' ); ?></a>
						</div>
					</article>
			...
			<?php else : ?>
				...
				<div class="text-center py-12 bg-brand-10 rounded-lg">
					<h3 class="h4 text-brand-black mb-2">
						<?php esc_html_e( 'No team members found', 'summit-law-theme' ); ?>
					</h3>
					<p class="text-brand-50 mb-6">
						<?php
						printf(
							/* translators: %s: role name */
							esc_html__( 'There are currently no %s listed.', 'summit-law-theme' ),
							esc_html( strtolower( $role_name ) )
						);
						?>
					</p>
					<a href="<?php echo esc_url( home_url( '/team/' ) ); ?>" class="btn">
						<?php esc_html_e( 'View All Team Members', 'summit-law-theme' ); ?>
					</a>
				</div>
