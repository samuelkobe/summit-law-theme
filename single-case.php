 ...
 *
 * Displays individual case posts with:
 * - Featured image banner with dark overlay
 * - Case title, date, and area taxonomy
...
<header class="case-banner relative bg-green-deep min-h-[300px] lg:min-h-[400px] flex items-end">
	...
	<div class="container mx-auto p-6 relative z-10 pb-12 lg:pb-16">
		...
		<nav class="mb-4 text-sm" aria-label="Breadcrumb">
			<ol class="flex items-center gap-2">
				<li>
					<a href="<?php echo esc_url( get_post_type_archive_link( 'case' ) ); ?>" class="text-white/70 hover:text-white transition-colors">
						Cases
					</a>
				</li>
				<li aria-hidden="true">
		...
		<div class="flex flex-wrap items-center gap-4 text-white/80 text-sm">
			...
			<?php if ( $areas && ! is_wp_error( $areas ) ) : ?>
				...
				<a href="<?php echo esc_url( add_query_arg( 'post_type', 'case', get_term_link( $areas[0] ) ) ); ?>" class="text-green-accent1 hover:underline transition-colors">
			...
			<div class="flex flex-wrap items-center justify-between gap-4">
				<?php if ( $areas && ! is_wp_error( $areas ) ) : ?>
					<div class="flex items-center gap-2 text-sm">
						...
						<a href="<?php echo esc_url( add_query_arg( 'post_type', 'case', get_term_link( $areas[0] ) ) ); ?>" class="text-green-deep hover:underline transition-colors">
							<?php echo esc_html( $areas[0]->name ); ?>
						</a>
					</div>
				<?php endif; ?>
				...
				<!-- Back to Cases link -->
				<a href="<?php echo esc_url( get_post_type_archive_link( 'case' ) ); ?>" class="text-green-deep hover:underline text-sm flex items-center gap-2 transition-colors">
